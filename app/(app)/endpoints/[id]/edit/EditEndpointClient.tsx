...
export function EditEndpointClient({ endpoint, namespace }: { endpoint: any, namespace: string }) {
  const [path, setPath] = useState(endpoint.path)
  const [method, setMethod] = useState(endpoint.method === 'ALL' ? 'GET' : endpoint.method)
  const [type, setType] = useState<'template'|'schema'>(endpoint.type)
  const [configStr, setConfigStr] = useState(() => JSON.stringify(endpoint.config, null, 2))
  const [activeEditor, setActiveEditor] = useState<'response' | 'request'>('response')
  const [enableValidation, setEnableValidation] = useState(!!endpoint.request_schema)
  const [requestSchemaStr, setRequestSchemaStr] = useState(() => {
    if (endpoint.request_schema) {
      return JSON.stringify(endpoint.request_schema, null, 2)
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="h-full flex flex-col">
      ...
      <div className="flex justify-end items-center mt-8 pt-6 border-t border-zinc-200 gap-4">
        <Link href={`/endpoints/${endpoint.id}`} className="px-5 py-2.5 text-sm font-semibold text-zinc-600 hover:text-zinc-900 transition-colors">Cancel</Link>
        <button type="submit" disabled={loading} className="flex items-center justify-center gap-2 min-w-[140px] bg-indigo-600 text-white px-8 py-2.5 rounded-lg text-sm font-semibold hover:bg-indigo-700 disabled:opacity-80 transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2">
          {loading ? (
             <>
